...
- **v2 2순위 데이터 포트: 계약 a~e 전부 확정(설계 완료)**, 구현 착수:
  ...
  - ✅ **d. 입력 통합** — `FieldSpec`→`InputSpec`(id/typeId/label/default/connectable/options). `NodeType.inputs()` 단일 소스(config 노브=connectable false / 데이터 입력=connectable true), 에디터·GraphParser 모두 `inputs`에서 읽음. 13노드 전부 `inputs()` 직접 선언(`InputSpec.knob`/`enumKnob`), `FieldSpec`·`SimpleNodeType`·`dataInPorts()`·스키마 `fields`/`dataIn` 제거. **타입 표기**(타입시스템 인수인계 문서): 값 원시 **bare**(`string/number/boolean`), 참조만 네임스페이스(`colophon:player`), 내부 `TypeId` sealed(Builtin/Named), `number`=단일 IEEE754 double. **커밋 3749085·ede6c99·5749cc1·9cb24fd·367e351·1f7bc73.** _(에디터 인라인폼 미연결=인라인/연결=와이어 UX는 데이터 입력 노드 생기는 e에서.)_
  - ⬜ **e. 다음** — 첫 데이터 노드(get_balance·get_variable·format_text·compare) + 트리거 명시 출력(victim/killer). "값이 흐른다" 실증(economy/state로). **여기서 c의 exec push/pure pull 실배선 + 데이터 엣지 그래프 배선.**
