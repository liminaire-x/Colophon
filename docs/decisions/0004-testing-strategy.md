...
- **2층은 보류(잠금 대기)** — 아래 "테스트 작성 트리거" 참조. 재개 시: 노드 1~2개
  (`player_info`·`get_variable`)를 `ServerPlayer`/`MinecraftServer`/`EconomyService`만
  Mockito로, 나머지는 실제 객체. 인메모리 `StorageService`를 `Fixtures`에 추가.

## 테스트 작성 트리거 (언제 붙이나)

테스트는 시간이 아니라 **계약이 잠기는 사건**에 붙인다. 이것이 "노드가 계속 바뀌는데
테스트를 언제 쓰나"의 답이다.

- **코어 계약 = 이미 잠김 → 1층 있음.** 값 흐름·파서·타입은 잠긴 계약([CLAUDE.md #4]),
  1층으로 커버됨. 코어가 진화해도 계약이 그대로면 안 깨진다.
- **노드 로직 = 유동 → 미룸.** 앞으로 구현하며 노드 입출력·시맨틱이 바뀔 여지가 크다.
  잠기기 전에 2층을 붙이면 재작성 비용이 눈덩이가 된다. **2층은 "다음 작업"이 아니라
  "계약 잠금 대기".** 재개 트리거: (a) 계약 f가 노드에 **안정 ID·version**을 부여하는 시점
  (version 붙은 노드 = 계약 잠김 = 테스트 대상), (b) 특정 노드를 명시적으로 "안정"으로 표시.
- **f(직렬화·마이그레이션) = 예외적 우선.** 노드 *로직*은 유동적이지만 노드 *직렬화 표면*
  (안정 ID·version·마이그레이션 규칙·/api/validate)은 잠긴 계약이다. 여기 버그는 **저장 데이터
  손상**이라 치명적이고 인게임 확인도 어렵다. → f 구현 시 라운드트립·버전 마이그레이션·validate
  케이스를 **1층급으로 동반**(노드 로직 테스트와 구분).
- **회귀 우선(regression-first).** 인게임에서 잡은 버그가 *안정 계약*(값 흐름·파서·직렬화)에서
  재현되면 재현 테스트를 남긴다. **노드 로직 버그는 계약이 유동 중이면 남기지 않는다**(곧 바뀔
  코드에 테스트를 고정하지 않는다).

## 구현 후 테스트 체크 (사후 게이트)

- **코어(`runtime`/`type`) 변경** → push → CI `:test` 그린 확인
  (`gh run view <id> --json conclusion`). 실패 시 fix-forward.
- **코어 계약을 의도적으로 바꿨으면**(드문 잠금 해제 결정) → **같은 커밋에서** 해당 1층 테스트를
  새 계약으로 갱신한다(테스트가 옛 행위를 붙잡아 리팩터를 막지 않게).
- **노드만 바꿨으면** 테스트 갱신 불필요(테스트 없음) → IntelliJ/인게임 동작 게이트로 확인.
