...
public final class Runner {
    ...
    public static void run(Graph graph, String startId, Context ctx) {
        ...
        while (current != null) {
            if (++steps > MAX_STEPS) {
                LOGGER.error("[Lorebench] Graph '{}' ran more than {} steps (a loop?); stopped", graph.id(), MAX_STEPS);
                return;
            }
            ...
            try {
                ...
                LOGGER.error("[Lorebench] Graph '{}' node '{}' threw; stopped", graph.id(), node.id(), e);
                return;
            }
            switch (result) {
                ...
                case NodeResult.Fail f -> {
                    LOGGER.warn("[Lorebench] Graph '{}' node '{}' failed: {}", graph.id(), node.id(), f.reason());
                    return;
                }
            }
        }
